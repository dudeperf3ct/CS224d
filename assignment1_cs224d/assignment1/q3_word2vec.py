...
def normalizeRows(x):
    ...
    N = x.shape[0]
    sum_row = np.sqrt(np.sum(x ** 2, axis=1))
    x = x  / sum_row.reshape(N, 1)
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    print (prob.shape, predicted.shape, outputVectors.shape)
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset, 
    ...
    ### YOUR CODE HERE
    #raise NotImplementedError
    #print (outputVectors.shape, predicted.shape)
    ...
    outputvecs = outputVectors[ids, :]    
    
    t = sigmoid(outputvecs.dot(predicted) * labels)
    cost = -np.sum(np.log(t))

    delta = labels * (t-1)
    gradPred = delta.reshape((1, K+1)).dot(outputvecs).flatten()
    gradtemp = delta.reshape((K+1, 1)).dot(predicted.reshape((1, predicted.shape[0])))

    for k in range(K):
        grad[ids[k]] += gradtemp[k, :]
