...
class RNN_Model():
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Composition'):
            ...
            W1 = tf.get_variable("Weight", [2*self.config.embed_size, self.config.embed_size])
            b1 = tf.get_variable("biases", [1, self.config.embed_size])
            ### END YOUR CODE
        with tf.variable_scope('Projection'):
            ### YOUR CODE HERE
            #pass
            U = tf.get_variable("Weight_2", [self.config.embed_size, self.config.label_size])
            bs = tf.get_variable("biases_2", [1, self.config.label_size])
            ### END YOUR CODE
    ...
    def add_model(self, node):
        ...
        with tf.variable_scope('Composition', reuse=True):
            ### YOUR CODE HERE
            #pass
            W1 = tf.get_variable("Weight")
            b1 = tf.get_variable("biases")
            word_embeddings = tf.get_variable("embedding")
            ### END YOUR CODE

        W_left, W_right = tf.split(W1, num_or_size_splits=2, axis=0)    
        node_tensors = dict()
        curr_node_tensor = None
        if node.isLeaf:
            ### YOUR CODE HERE
            #pass
            word_id = self.vocab.encode(node)
            curr_node_tensor = tf.expand_dims(tf.gather(word_embeddings, word_id), 0)
            ### END YOUR CODE
        else:
            ...
            ### YOUR CODE HERE
            #pass
            curr_node_tensor = tf.matmul(node_tensors[node.left], W_left) + \
                                tf.matmul(node_tensors[node.right], W_right) + b1
            ### END YOUR CODE
    ...
    def add_projections(self, node_tensors):
        ...
        with tf.variable_scope("Projection", reuse=True):
            U = tf.get_variable("Weight_2")
            bs = tf.get_variable("biases_2")
            logits = tf.nn.softmax(tf.matmul(node_tensors, U) + bs)
        ### END YOUR CODE
        return logits
    ...
    def loss(self, logits, labels):
        ...
        with tf.variable_scope("Composition", reuse=True):
            W1 = tf.get_variable("Weight")
            b1 = tf.get_variable("biases")
            l2_loss = self.config.l2 * (tf.nn.l2_loss(W1) + tf.nn.l2_loss(b1))
